    ...
    <section className="py-6 md:py-10 bg-white">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto">
          <div className="prose prose-base max-w-none prose-headings:font-serif prose-headings:text-navy prose-p:text-charcoal/80 prose-a:text-gold prose-a:no-underline hover:prose-a:underline">
            <ReactMarkdown
              ...
              components={{
                ...
                code: ({node, inline, className, children, ...props}) => {
                  ...
                  // Only apply code block styling to real code blocks
                  const match = /language-(\w+)/.exec(className || '');
                  return (
                    <div className="my-4">
                      <code className={`block bg-slate-100 p-4 rounded text-navy/90 text-sm overflow-x-auto ${match ? className : ''}`} {...props}>
                        {children}
                      </code>
                    </div>
                  );
