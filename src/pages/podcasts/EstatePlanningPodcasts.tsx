...
import { ChevronLeft, PlayCircle, Filter, Headphones, Clock, Calendar } from 'lucide-react';
import ConsultationCTA from "../../components/ConsultationCTA";
import ResourcePageHeader from "../../components/resources/ResourcePageHeader";
import { podcastData } from "../../data/podcastData";
import { Toaster } from "@/components/ui/toaster";

// Filter to get only Estate Planning related podcasts
const estatePlanningPodcasts = podcastData.filter(
  podcast => podcast.topics.some(topic => 
    ['Estate Planning', 'Trusts', 'Probate', 'Inheritance', 'Family Governance', 'Wealth Transfer'].includes(topic)
  )
);

const EstatePlanningPodcasts: React.FC = () => {
  const [isFilterOpen, setIsFilterOpen] = useState(false);
  const [selectedTopics, setSelectedTopics] = useState<string[]>([]);
  const [sortOrder, setSortOrder] = useState<'newest' | 'oldest'>('newest');
  
  // Get all unique topics within estate planning podcasts
  const allTopics = Array.from(
    new Set(estatePlanningPodcasts.flatMap(podcast => podcast.topics))
  );
  ...
  const filteredAndSortedPodcasts = estatePlanningPodcasts
    .filter(podcast => 
  ...
  return (
    <main className="pt-48 bg-white">
      <ResourcePageHeader 
        title="Estate Planning Podcasts"
        description="Listen to in-depth discussions on estate planning, wealth transfer, trust administration, and inheritance planning strategies."
        variant="yellow"
      />
                ...
                >
                  ...
                  <div className="h-32 relative border-b-2 border-black overflow-hidden">
                    ...
                    <div className="absolute inset-0 grid grid-cols-12 grid-rows-6 gap-1 p-1">
                      {index % 3 === 0 ? (
                        <>
                          <div className="col-span-8 row-span-3 bg-yellow-400 border border-black"></div>
                          <div className="col-span-4 row-span-6 bg-white border border-black"></div>
                          <div className="col-span-8 row-span-3 bg-white border border-black"></div>
                        </>
                      ) : index % 3 === 1 ? (
                        <>
                          <div className="col-span-4 row-span-6 bg-white border border-black"></div>
                          <div className="col-span-8 row-span-2 bg-blue-600 border border-black"></div>
                          <div className="col-span-8 row-span-2 bg-yellow-400 border border-black"></div>
                          <div className="col-span-8 row-span-2 bg-white border border-black"></div>
                        </>
                      ) : (
                        <>
                          <div className="col-span-3 row-span-6 bg-red-600 border border-black"></div>
                          <div className="col-span-9 row-span-3 bg-white border border-black"></div>
                          <div className="col-span-9 row-span-3 bg-yellow-400 border border-black"></div>
                        </>
                      )}
                    </div>
      ...
      <section className="py-16 bg-yellow-50 border-t-4 border-black">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <div className="flex flex-col md:flex-row items-center gap-8">
              <div className="md:w-1/3">
                <div className="h-64 w-64 mx-auto bg-white border-2 border-black p-4">
                  <div className="h-full w-full bg-gradient-to-br from-yellow-400 to-yellow-600 flex items-center justify-center">
              ...
              <div className="md:w-2/3">
                ...
                <p className="text-gray-700 mb-6">
                  Our podcast series has partnered with the Estate Planning Academy to bring you expert insights and advanced strategies. Subscribe to our newsletter for exclusive content and early access to new episodes.
                </p>
...
export default EstatePlanningPodcasts; 